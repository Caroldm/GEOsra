...
my $url = $base . 'esearch.fcgi?db=gds&term=(homo sapiens[ORGN]+OR+Mus musculus[ORGN]+OR+Rattus norvegicus[ORGN])+AND+gse[ETYP]+AND+sra[suppFile]&retmax=500000&usehistory=y';
#my $url = $base . 'esearch.fcgi?db=gds&term=(homo sapiens[ORGN]+OR+Mus musculus[ORGN])+AND+gse[ETYP]+AND+sra[suppFile]&retmax=500000&usehistory=y';
...
#post the efetch URL
$data = get($url);
$dataout = getstore( $url, "geo_sra.txt")
#print "$data";
